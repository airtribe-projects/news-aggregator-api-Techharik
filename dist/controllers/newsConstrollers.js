...
import { articleCahce, fetchPostOnPreferences, fetchSequentially } from "../utils/NewsFetcher.js";
// inmemoryCache: 
// "preference": [{id},{id}]
...
const postNewsRead = asyncHandler(async (req, res) => {
...
const postNewsFavourite = asyncHandler(async (req, res) => {
    const id = req.params.id;
    let found = false;
    for (const cache of articleCahce.values()) {
        const article = cache.find((a) => a.id === id);
...
const getRead = asyncHandler(async (req, res) => {
    const readArticles = [];
    for (const cache of articleCahce.values()) {
